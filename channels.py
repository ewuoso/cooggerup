...
# easy
from easysteem.easysteem import Oogg, EasyFollow, EasyAccount, EasyPost

Oogg = Oogg(node = None)
EasyPost = EasyPost(node = None)
...
class Coogger(MainInit):
    ...
    async def follow(self):
        follow_class = EasyFollow(username = self.mscs[1],node = None)
        ...
        try:
            await self.sendms(self.mschannel,str(context))
        except:
    ...
    async def post(self):
        count_rshares = 0.0
        for votes_result in EasyPost.votes_by_rshares(url = self.mscs[1]):
        ...
        sbd_sp = EasyPost.calculate_sbd_sp(float(count_rshares))
        await self.sendms(self.mschannel, "**${}  --  ${}.SP  --  ${}.SBD**".format(round(count_rshares,4), sbd_sp["sp"],sbd_sp["sbd"]))

    async def balance(self):
        result = EasyAccount(username = self.mscs[1],node = None).account_balance()
        for i in result:
            i = str(i),(result[i])
    ...
    async def account(self):
        account = EasyAccount(self.mscs[1],node = None)
        result = dict(
    ...
    async def payout(self):
        ea = EasyAccount(self.mscs[1],node = None)
        sbd_in_account = ea.account_balance()["SBD"]
        total_sbd = sbd_in_account
        for pp in EasyPost.pending_payout(self.mscs[1]):
        ...
        total_usd = round(total_sbd * float(Oogg.price()["SBD"]),4),
        )
        for i in result:
            i = str(i),(result[i])
            await self.sendms(self.mschannel,str(i))
    ...
    async def calculate(self):
        sbd_sp = EasyPost.calculate_sbd_sp(float(self.mscs[1]))
        await self.sendms(self.mschannel, "sonuçlar bitti ${}.sbd   ${}.sp".format(sbd_sp["sbd"],sbd_sp["sp"]))
